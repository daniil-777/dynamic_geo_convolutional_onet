...
# Transforms
class PointcloudNoiseRandomStdDev(object):
    ''' Point cloud noise transformation class.

    It adds noise to point cloud data with random stddev
    ...
    def __init__(self, stddev):
        self.stddev_upper_bound = stddev
    ...
    def __call__(self, data):
        ...
        data_out = data.copy()
        points = data[None]
        noise = np.random.uniform(0, self.stddev_upper_bound) * np.random.randn(*points.shape)
        noise = noise.astype(np.float32)
        data_out[None] = points + noise
        return data_out
