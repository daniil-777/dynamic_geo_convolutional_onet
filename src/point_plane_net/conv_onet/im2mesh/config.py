...
from im2mesh import onet, r2n2, psgn, pix2mesh, dvr
...
def get_dataset(mode, cfg, return_idx=False, return_category=False):
    ...
    if 'overfit' in cfg['data']:
        flag_overfit = cfg['data']['overfit']
        overfit_num = cfg['data']['overfit_num']
    else:
        flag_overfit = False
        overfit_num = None
    ...
    if dataset_type == 'Shapes3D':
        ...
        dataset = data.Shapes3dDataset(
            ...
            overfit=flag_overfit,
            overfit_num=overfit_num
        )
        ...
        dataset = data.Shapes3dDatasetDVR(
            dataset_folder, fields,
            split=split,
            categories=categories,
        )
...
def get_inputs_field(mode, cfg):
    ...
    if input_type is None:
        ...
        if not cfg['data']['noise_random_stddev']:
            transform = transforms.Compose([
            ...
            transform = transforms.Compose([
            data.SubsamplePointcloud(cfg['data']['pointcloud_n']),
            data.PointcloudNoiseRandomStdDev(cfg['data']['pointcloud_noise'])
            ])
        with_transforms = cfg['data']['with_transforms']
