...
def sample_patch_points(batch_size, n_points, patch_size=1,
    ...
    Args:
        ...
        continuous (bool): whether to sample continuously or only on pixel
            locations
    '''
    # assume image range of [-1, 1] for now
    assert(patch_size > 0)
...
def get_occupancy_loss_points(pixels, camera_mat, world_mat, scale_mat,
    ...
    Args:
        ...
        max_depth_value (float): maximum depth values; important when no cube
    ...
    d_occupancy = torch.rand(batch_size, n_points).to(device) * max_depth_value
    ...
    if occupancy_random_normal:
        d_occupancy = torch.randn(batch_size, n_points).to(device) \
            * (max_depth_value / 8) + max_depth_value / 2
        if use_cube_intersection:
...
def get_freespace_loss_points(pixels, camera_mat, world_mat, scale_mat,
                              use_cube_intersection=True, max_depth_value=2.4):
