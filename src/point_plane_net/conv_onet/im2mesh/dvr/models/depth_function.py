...
class DepthModule(nn.Module):
    ...
    Args:
        ...
        d_range (tuple): range of possible depth values; not relevant when
    ...
    def __init__(self, tau=0.5, n_steps=[128, 129], n_secant_steps=8,
        ...
        #self.check_cube_intersection = False  # TODO Change at some point!
    ...
    def forward(self, ray0, ray_direction, decoder, z=None, c=None, it=None,
                n_steps=None):
        ...
        to obtain the "default" depth value as the z-axis of the transformed
        point.)
        ...
        if n_steps[1] > 1:
            if type(c) == dict:
                inputs = [ray0, ray_direction, decoder, z, len(c)] + [k for k in c.keys()] + [v for v in c.values()] + [n_steps,
                        self.n_secant_steps, self.tau, self.d_range,
                        self.method, self.check_cube_intersection,
                        self.max_points] + [k for k in decoder.parameters()]                
            else:
                inputs = [ray0, ray_direction, decoder, z, 1, '', c, n_steps,
                        self.n_secant_steps, self.tau, self.d_range,
                        self.method, self.check_cube_intersection,
                        self.max_points] + [k for k in decoder.parameters()]
...
class DepthFunction(torch.autograd.Function):
    ...
    def run_Bisection_method(d_low, d_high, n_secant_steps, ray0_masked,
                             ray_direction_masked, decoder, z, c, logit_tau):
        ...
        for i in range(n_secant_steps):
            ...
            with torch.no_grad():
                #f_mid = decoder(p_mid, z, c, batchwise=False, only_occupancy=True) - logit_tau
                f_mid = decoder(p_mid, z, c, only_occupancy=True) - logit_tau
            ind_low = f_mid < 0
    ...
    def run_Secant_method(f_low, f_high, d_low, d_high, n_secant_steps,
                          ray0_masked, ray_direction_masked, decoder, z, c,
                          logit_tau):
    ...
    @staticmethod
    def perform_ray_marching(ray0, ray_direction, decoder, z=None, c=None,
                             tau=0.5, n_steps=[128, 129], n_secant_steps=8,
                             d_range=[0., 2.4], method='secant',
        ...
        d_proposal = torch.linspace(
            d_range[0], d_range[1], steps=n_steps).view(
        ...
        # Get depth values and function values for found the interval
        ...
        if method == 'secant' and mask.sum() > 0:
            ...
            d_pred = torch.ones(batch_size, n_pts).to(device)

        # for sanity
        pt_pred = torch.ones(batch_size, n_pts, 3).to(device)
        pt_pred[mask] = ray0[mask] + \
            d_pred.unsqueeze(-1)[mask] * ray_direction[mask]
        # for sanity
        d_pred_out = torch.ones(batch_size, n_pts).to(device)
        d_pred_out[mask] = d_pred[mask]
    ...
    def forward(ctx, *input):
        ...
        '''
        ray0, ray_direction, decoder, z, len_c = input[:5]
        c_keys = input[5:5+len_c]
        c_values = input[5+len_c:5+len_c+len_c]
        n_steps, n_secant_steps, tau, d_range, method, check_cube_intersection, max_points = input[5+len_c+len_c:12+len_c+len_c]


        if len(c_keys) == 1 and c_keys[0] == '':
            assert(len(c_values) == 1)
            c = c_values[0]
        else:
            c = {}
            for i in range(len(c_keys)):
                c[c_keys[i]] = c_values[i]


        # (ray0, ray_direction, decoder, z, c, n_steps, n_secant_steps, tau,
        #  d_range, method, check_cube_intersection, max_points) = input[:12]

        # Get depth values
        with torch.no_grad():
            d_pred, p_pred, mask, mask_0_not_occupied = \
                DepthFunction.perform_ray_marching(
                    ray0, ray_direction, decoder, z, c, tau, n_steps,
                    n_secant_steps, d_range, method, check_cube_intersection,
                    max_points)
    ...
    def backward(ctx, grad_output):
        ...
        gradients. As for every input a gradient has to be returned, we return
        ...
        with torch.enable_grad():
            ...
            f_p = decoder(p_pred, z, c, only_occupancy=True)
            f_p_sum = f_p.sum()
            ...
            if type(c) == dict and mask.sum() == 0:
                gradc = [None for i in range(len(c))]
            elif c is None or ((not type(c) == dict) and c.shape[-1] == 0) or mask.sum() == 0:
                gradc = [None]
            else:
                if type(c) == dict:
                    gradc = torch.autograd.grad(f_p, [v for v in c.values()], retain_graph=True,
                            grad_outputs=grad_outputs)
                else:
                    gradc = torch.autograd.grad(f_p, c, retain_graph=True,
                                                grad_outputs=grad_outputs)#[0]

            # Gradients for latent code z
            if z is None or z.shape[-1] == 0 or mask.sum() == 0:
                gradz = None
            else:
                gradz = torch.autograd.grad(f_p, z, retain_graph=True,
                                            grad_outputs=grad_outputs)[0]
        ...
        # # out = [None, None, None, gradz, gradc, None, None, None, None, None,
        #        None, None] + list(grad_phi)
        if type(c) == dict:
            grad_c_keys = [None for i in range(len(gradc))]
            out = [None, None, None, gradz, None] + list(grad_c_keys) + list(gradc) + [None, None, None, None, None, None, None] + list(grad_phi)
        else:
            out = [None, None, None, gradz, None, None] + list(gradc) + [None, None, None, None, None, None, None] + list(grad_phi)
        return tuple(out)
