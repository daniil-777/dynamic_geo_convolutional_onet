...
class Trainer(BaseTrainer):
    ...
    Args:
        ...
        depth_from_visual_hull (bool): whether to use depth from visual hull
            for occupancy loss
        max_depth_value (float): maximal depth value; if cube intersection is
            used this value is relevant
    ...
    def __init__(self, model, optimizer,
                 device=None, vis_dir=None, threshold=0.5,
                 n_training_points=2048, dataset_name='DVRShapes',
                 lambda_occupied=1., lambda_freespace=1., generator=None,
                 lambda_rgb=1., n_eval_points=4000,
                 lambda_depth=0., lambda_image_gradients=0., patch_size=1,
                 reduction_method='sum', sample_continuous=False,
                 lambda_sparse_depth=0., overwrite_visualization=True,
                 depth_from_visual_hull=False, max_depth_value=2.4,
                 depth_loss_on_world_points=False,
                 occupancy_random_normal=False,
                 use_cube_intersection=False, always_freespace=True,
                 multi_gpu=False, lambda_normal=0., 
                 lambda_iou=0., **kwargs):
        self.model = model
        if multi_gpu:
        ...
        self.use_cube_intersection = False  # TODO Change this

        self.lambda_iou = lambda_iou

        # TODO add this to the configs and remove dataset name argument
        if self.dataset_name == 'DTU':
            print('Warning: Changing settings because of DTU dataset ...')
            self.max_depth_value = 1400
            self.depth_loss_on_world_points = True
        if self.dataset_name == 'NMR':
            print('Warning: Changing settings because of NMR dataset ...')
            self.max_depth_value = 5.
    ...
    def calc_depth_loss(self, mask_depth, depth_img, pixels,
        ...
        if self.lambda_depth != 0 and mask_depth.sum() > 0:
            ...
            '''
            I added this mask because it is necessary for our new project;
            maybe we can make it nicer, but works for now
            '''
            mask_depth_not_zero = depth_gt != 0
            mask_depth &= mask_gt_depth
            mask_depth &= mask_depth_not_zero
            if self.depth_loss_on_world_points:
                ...
                # new - mask if d_pred and d_gt are too different
                #eps_diff = 2.
                #mask_diff = (d_pred - depth_gt).abs() < eps_diff
                #mask_diff[mask_depth==0] = False
                #mask_depth &= mask_diff
                # mask_diff = None
                # eps_diff = 0.025
                # mask_diff = mask_depth & ((d_pred - depth_gt).abs() > eps_diff)
                # mask_depth = mask_depth & ((d_pred - depth_gt).abs() <= eps_diff)
                mask_diff = None
            ...
            return mask_depth_not_zero, mask_diff
        return None, None
    ...
    def calc_normal_loss(self, normals, batch_size, loss={}, eval_mode=False):
        ...
        if self.lambda_normal != 0:
            laplace_loss = torch.norm(normals[0] - normals[1], dim=-1).sum() *\
                self.lambda_normal / batch_size
            loss['loss'] += laplace_loss
            loss['laplace_loss'] = laplace_loss
            if eval_mode:
                laplace_loss_eval = torch.norm(
                    normals[0] - normals[1], dim=-1).mean() * \
                    self.lambda_normal
                loss['laplace_loss_eval'] = laplace_loss_eval
    ...
    def compute_loss(self, data, eval_mode=False, it=None):
        ...
        mask_gt = get_tensor_values(
            mask_img, p, squeeze_channel_dim=True).bool() #.type(torch.uint8)
        ...
        if self.lambda_iou > 0:
            p_iou = data.get('points').to(device) # NEW
        else:
            p_iou = None
        ...
        (p_world_hat, rgb_pred, logits_occupancy, logits_freespace, mask_pred,
            p_world_hat_sparse, mask_pred_sparse, normals, mask_cam_occupied, logits_iou) = self.model(
                p, p_occupancy, p_freespace, inputs, camera_mat, world_mat,
                scale_mat, it, sparse_depth, self.lambda_normal != 0, p_iou)

        # 4.) Calculate Loss
        # 4.1) Photo Consistency Loss
        # rgb_pred = self.model.decode_color(p_world_hat, z=z, c=c)
        ...
        mask_depth_not_zero, mask_diff = self.calc_depth_loss(mask_depth, depth_img, p, camera_mat,
        ...
        # 4.5) Freespace loss
        #mask_freespace = ((mask_gt == 0) | mask_cam_occupied) if self.always_freespace else ((mask_gt == 0) & (mask_pred))
        mask_freespace = mask_cam_occupied
        ...
        if self.lambda_iou != 0:
            # NEW LOSS TEST
            p_occ = data.get('points.occ').to(device)    
            l = torch.nn.functional.binary_cross_entropy_with_logits(logits_iou, p_occ, reduction='none').sum()
            loss['loss'] += l
            loss['loss_iou'] = l

        mask_occupancy = ((mask_pred == 0) & mask_gt) 
        if mask_diff is not None:
            mask_occupancy |= mask_diff  #(mask_gt & (mask_diff == 0))
        if mask_depth_not_zero is not None:
            mask_occupancy &= mask_depth_not_zero
    ...
    def visualize(self, data, it=0, vis_type='voxel'):
        ''' Visualized the data.
        ...
        if vis_type == 'voxel':
            ...
            p = make_3d_grid([-0.48] * 3, [0.48] * 3, shape).to(device)
