...
import time
from im2mesh.dvr_common import transform_pointcloud
...
class Generator3D(object):
    ...
    Args:
        ...
        refine_max_faces (int): max number of faces which are used as batch
            size for refinement process
    '''

    def __init__(self, model, points_batch_size=100000,
                 threshold=0.5, refinement_step=0, device=None,
                 resolution0=16, upsampling_steps=3,
                 with_normals=False, padding=0.1, sample=True,
    ...
    def generate_mesh(self, data, return_stats=True):
        ...
        mesh = self.generate_from_latent(z, c, stats_dict=stats_dict,
                                         data=data, **kwargs)
    ...
    def generate_meshes(self, data, return_stats=True):
        ...
        for i in range(inputs.shape[0]):
            ...
            z = self.model.get_z_from_prior((1,),
    ...
    def generate_from_latent(self, z=None, c=None, stats_dict={}, data=None,
                             **kwargs):
    ...
    def extract_mesh(self, occ_hat, z, c=None, stats_dict=dict()):
        ...
        if self.with_color and not vertices.shape[0] == 0:
            t0 = time.time()
            vertex_colors = self.estimate_colors(np.array(mesh.vertices), z, c)
        ...
        for vi in vertices_split:
            ...
            with torch.no_grad():
                ci = self.model.decode_color(
                    vi.unsqueeze(0), z, c).squeeze(0).cpu()
            colors.append(ci)
        ...
        while it_r < self.refinement_step:
            for f_it in dataloader:
                ...
                face_value = torch.cat([
                    torch.sigmoid(self.model.decode(p_split, z, c).logits)
                ...
                # print('Refinement loss (times 100) (%d it): %.03f' %
                #       (it_r, loss * 100))
