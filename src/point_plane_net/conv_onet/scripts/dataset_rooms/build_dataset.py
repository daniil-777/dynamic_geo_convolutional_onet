...
output_path = '/is/rg/avg/mniemeyer/project_data/2020/scalable_onet/data/RoomDataset6'
output_path_scene = '/is/rg/avg/mniemeyer/project_data/2020/scalable_onet/data/scenes6'
...
n_objects = [8, 7, 6, 5, 4]  # these will be considered individual classes! used for ds5
n_objects = [4, ]  # these will be considered individual classes!
#scale_interval = [0.25, 0.45]  RoomDataset3
#scale_interval = [0.1, 0.23]  # for new RoomDataset4
scale_intervals = [ # used dfor ds5
    [0.1, 0.25],
    [0.15, 0.25],
    [0.2, 0.30],
    [0.25, 0.40],
...
xz_groundplane_range = [0.3, 1.]  # new for RoomDataset4
y_angle_range = [0., 360.]
wall_probabilities = [0.5, 0.5, 0.5, 0.5]
wall_thickness = 0.01
wall_height_range = [0.2, 0.4]
lambda_wall_ground = 0.2  # new for RoomDataset5
...
def sample_pointcloud(mesh_list, cl_list, walls, xz_range, ground_plane_only_above=True, walls_only_side=True):
    ...
    # if ground_plane_only_above:
    #     mask &= points[:, 1] > -0.5 + groundplane_thickness - 1e-4
    if walls_only_side:
        if walls[0] == 1:
...
for n_class, n_object in enumerate(n_objects):
    ...
    for split_idx, split_name in enumerate(split_names):
        ...
        while split_item_idx < n_rooms[split_idx]:
            ...
            item_dict['xz_groundplane_range'] = np.array(scales) #np.random.rand(1) * (xz_groundplane_range[1] - xz_groundplane_range[0]) + xz_groundplane_range[0]
