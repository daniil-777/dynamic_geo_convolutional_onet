    ...
    for batch in train_loader:
        ...
        if print_every > 0 and (it % print_every) == 0:
            print('[Epoch %02d] it=%03d, loss=%.4f'
                  % (epoch_it, it, loss))
            #print(f'Current plane weight {model.encoder.fc_plane_net.state_dict()}')
        ...
        if validate_every > 0 and (it % validate_every) == 0:
            #print(f'Current plane weight {model.encoder.plane_weights}')
            ...
            for k, v in eval_dict.items():
                logger.add_scalar('val/{model_name}/%s' % k, v, it)
