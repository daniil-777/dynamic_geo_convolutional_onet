...
class OccupancyNetwork(nn.Module):
    ...
    def forward(self, p, inputs, sample=True, **kwargs):
        ...
        '''
        batch_size = p.size(0)
        c, C_mat, plane_normals = self.encode_inputs(inputs)
        z = self.get_z_from_prior((batch_size,), sample=sample)
        p_r = self.decode(p, z, c, C_mat, **kwargs)
        return p_r
    ...
    def encode_inputs(self, inputs):
        ...
        '''
        # if self.encoder == 'sphere_net':
        #     print("spherenet")
        if self.encoder is not None:
            # c, C_mat, plane_normals = self.encoder(inputs)
            c, C_mat = self.encoder(inputs)
        else:
            # Return inputs?
            # c, C_mat, plane_normals = torch.empty(inputs.size(0), 0)
            c, C_mat = torch.empty(inputs.size(0), 0)

        return c, C_mat

    def decode(self, p, z, c, C_mat, **kwargs):
        ...
        logits = self.decoder(p, z, c, C_mat, **kwargs)
