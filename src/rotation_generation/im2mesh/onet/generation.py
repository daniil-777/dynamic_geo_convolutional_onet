...
class Generator3D(object):
    ...
    def generate_mesh(self, data, return_stats=True):
        ...
        mesh = self.generate_from_latent(z, c, C_mat, stats_dict=stats_dict, **kwargs)
    ...
    def generate_rotated_mesh(self, data, return_stats=True, DEGREES = 0, save_rotation_tensor = False):
        ...
        # random_counter = int(random.random()* 10000000000000 % 1)
        if(random_counter == 0):
            print('Found one!')
            fig = plt.figure()
    ...
    def generate_from_latent(self, z, c=None, C_mat=None, stats_dict={}, **kwargs):
        ...
        if self.upsampling_steps == 0:
            ...
            while points.shape[0] != 0:
                ...
                values = self.eval_points(
                    pointsf, z, c, C_mat, **kwargs).cpu().numpy()
        ...
        mesh = self.extract_mesh(value_grid, z, c, C_mat, stats_dict=stats_dict)
        return mesh
    ...
    def eval_points(self, p, z, c=None, C_mat=None, **kwargs):
        ...
        for pi in p_split:
            ...
            with torch.no_grad():
                occ_hat = self.model.decode(pi, z, c, C_mat, **kwargs).logits
    ...
    def extract_mesh(self, occ_hat, z, c=None, C_mat=None, stats_dict=dict()):
        ...
        if self.with_normals and not vertices.shape[0] == 0:
            t0 = time.time()
            normals = self.estimate_normals(vertices, z, c, C_mat)
        ...
        if self.refinement_step > 0:
            t0 = time.time()
            self.refine_mesh(mesh, occ_hat, z, c, C_mat)
            stats_dict['time (refine)'] = time.time() - t0

        return mesh

    def estimate_normals(self, vertices, z, c=None, C_mat=None):
        ...
        for vi in vertices_split:
            ...
            vi.requires_grad_()
            occ_hat = self.model.decode(vi, z, c, C_mat).logits
            out = occ_hat.sum()
            out.backward()
            ni = -vi.grad
    ...
    def refine_mesh(self, mesh, occ_hat, z, c=None, C_mat=None):
        ...
        for it_r in trange(self.refinement_step):
            ...
            face_value = torch.sigmoid(
                self.model.decode(face_point.unsqueeze(0), z, c, C_mat).logits
            )
    ...
    def rotate_points(self, pointcloud_model,query_points = False, DEGREES = 0, save_rotation_tensor = False):
        ...
        if(query_points != True):
            ...
            if(save_rotation_tensor):
                #used for experiemnt on rotating manually rotated planes, not included in report, experiment code
