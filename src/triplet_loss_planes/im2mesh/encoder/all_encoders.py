...
class ResnetVoxelSpherePointHeightnet(nn.Module):
    ...
    def forward(self, p):
        ...
        for l in range(3):
            ...
            current_net = net.where(mask_net, torch.tensor(0.0))
            
            # print("shape mask", mask.shape)
            print("p shape", p.shape)
            p_volume_selected = p.where(mask_p, torch.tensor(0.0))
            # print("mask", mask)
...
class ResnetPointHeightnet(nn.Module):
   ...
   #     n_channels (int): number of planes for projection
    ...
    def forward(self, p):
        ...
        net_pl = self.fc_plane_hdim(self.actvn(net_pl))
        net_pl = net_pl.unsqueeze(1) # batch_size x 1 x hidden_dim
        # print("first net shape", net.shape)
        # print("first net_pl shape", net_pl.shape)
        # Combine net and net_pl
        net = net + net_pl # to allow backpropagation to net_pl        
        ...
        for l in range(C_mat.size()[1]):
            ...
            # print("net before shape", net.shape)
            # out, _ = scatter_max(net, cell_index, dim=1, out=out)
            # print("out shape", out.shape)
            # print("net shape", net.shape)
            # print("------------------------------------")
            # print("out before shape", out_heights.shape)
            # print("net before shape", heights.shape)
            ...
            #add scatter min and scatter mean!!!!
            ############cincatenation of features from pointnet and heiaghmap
            # c[:,l,] = torch.cat((out.view(batch_size, H, W, self.hidden_dim), 
            #                     out_heights.view(batch_size, H, W, 1)),  dim=3)
            #32 (from pointnet + 3 (comes from [min, mean, max]))
            #so, we have information from pointnet and height map for every learned plane.

            #now we have a distance from the point till the plane going through [0,0,0]
            #
...
class ResnetMultiSpherePointnet(nn.Module):
    ...
    def forward(self, p):
        ...
        for l in range(sphere_centers.size()[1]):
            # print("shape plane_parameters[:,l,0]", sphere_parameters[:,l,0].shape)
            # print("shape p", p[:,:,0].shape)
            ...
            # x_v = p[:,:,0] - sphere_parameters[:,l,0]
            # y_v = p[:,:,1] - sphere_parameters[:,l,1]
            # z_v = p[:,:,2] - sphere_parameters[:,l,2]
            radios_distances = torch.norm(p, dim = 2).unsqueeze(2)
            #calculate the distance from the center to points
            latitudes = (90 - torch.atan2(z_v, torch.sqrt(x_v**2 + y_v**2))* 180/pi) #from north to the south from 0 to 180
            meridians = (360 + torch.atan2(y_v, x_v)* 180/pi) % 360 #counter-clockwise from 0 to 360
