...
########################################################################
#this file containes decoders for plane and spheres
########################################################################

def BillinearInterpolationMultiVoxelSphere(meridian, latitude, rad_dim, p, net, max_dim, device='cuda'):
    '''Applies bilinear interpolation to the feature grid from encoder with sphere way of discr with latitudes/meridians/depth-layers
    ...
    Parameters
    ----------
    p:        torch.tensor (batch_size, num_of_points, dimension)
               points samped during decoder stage 
    net:      toch.tensor (batch_size, n_channels, H, W, c_dim)
               U-net feature
    
    sphere_centers: torch.tensor (Batch, n_channels, 3)
    meridian: int
               number of meridians
    latitude: int 
               number of latitudes
    ...
    Returns
    ----------
    interpolated_feature: torch.tensor (batch_size, n_channels, H, W, c_dim)
                          interpolated U-net feature
    '''
    #decoder in case of sphere's discretisation by meridians/latitudes/depth-layers
    # p: points (batch_size, num_of_points, dimension)
    # net: U-net feature
    # meridian - number of meridians
    # latitude - number of latitudes
    # rad_dim  - number of depth-layers
    ...
    for l in range(rad_dim):
        ...
        ###that is the fast version for training#############
        # x_left = (x_grid - 1) % W
        # x_right = (x_grid + 1) % W
...
def BillinearInterpolationVoxelSphere(meridian, latitude, p, net, max_dim, device='cuda'):
    # p: points (batch_size, num_of_points, dimension)
    # c: U-net feature
...
def BillinearInterpolationMultiSphere(meridian, latitude, p, net, sphere_centers, max_dim, device='cuda'):
    '''Applies bilinear interpolation to the feature grid from encoder with sphere way of discr for different spheres
    ...
    interpolated_feature: torch.tensor (batch_size, n_channels, H, W, c_dim)
                          interpolated U-net feature
    '''
    #decoder in case of multiple spheres (sphere's discretisation by meridians/latitudes)
    # p: points (batch_size, num_of_points, dimension)
    # net: U-net feature
    # meridian - number of meridians
    # latitude - number of latitudes
    ...
    for l in range(sphere_centers.size()[1]):
        ...
        # x_v = p[:,:,0] - sphere_centers[:,l,0]
        # y_v = p[:,:,1] - sphere_centers[:,l,1]
        # z_v = p[:,:,2] - sphere_centers[:,l,2]
        latitudes = (90 - torch.atan2(z_v, torch.sqrt(x_v**2 + y_v**2))* 180/pi) #from north to the south from 0 to 180
        meridians = (360 + torch.atan2(y_v, x_v)* 180/pi) % 360 #counter-clockwise from 0 to 360
...
def BillinearInterpolationSphere(meridian, latitude, p, net, max_dim, device='cuda'):
    ...
    '''Applies bilinear interpolation to the feature grid from encoder with sphere way of discr
    ...
    for l in range(1):
        ...
        # y_high = (y_grid + 1) - (y_grid + 1)// H
        ######################################################
        #coords of left/right/low/high neighbours
        x_left = x_grid - 1
        x_right = x_grid + 1
        y_low = y_grid - 1
        y_high = y_grid + 1
...
class Decoder(nn.Module):
    ...
    def forward(self, p, z, c=None, **kwargs):
        ...
        out = out.squeeze(-1)
        # print("shape decoder out", out.shape)
        return out
...
class DecoderCBatchNorm(nn.Module):
    ...
    def forward(self, p, z, c, C_mat, **kwargs):
        ...
        out = out.squeeze(-1)
        # print("output shape of Decoder", out.shape)
        return out
...
class DecoderSphere(nn.Module):
    ...
    Args:
        ...
        z_dim (int): dimension of latent code z
        c_dim (int): dimension of latent conditioned code c
        meridian (int): number of meridians
        latitude (int): number of latitudes
        rad_dim (int): number of depth-layers
        hidden_size (int): hidden size of Decoder network
        leaky (bool): whether to use leaky ReLUs
        legacy (bool): whether to use the legacy structure
    '''
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=32,
                 hidden_size=256, meridian = 64, latitude = 64, rad_dim = 5, interpolation = 'one_sphere'):
        super().__init__()
        self.z_dim = z_dim
        if not z_dim == 0:
            self.fc_z = nn.Linear(z_dim, hidden_size)
    ...
    def forward(self, p, z, c, C_mat, **kwargs):
        ...
        interval = float(2 / (H-1))
        #chain of ifelse consition depending on the model choice:
        if (self.interpolation == 'multi_spheres'):
            c = BillinearInterpolationMultiSphere(self.meridian, self.latitude, p, c, C_mat, max_dim) #here C_mat is sphere centers
        elif (self.interpolation == 'one_sphere'):
            c = BillinearInterpolationSphere(self.meridian, self.latitude, p, c, max_dim) #hee C_mat is just a dummy tensor meaning nothing
        elif (self.interpolation == "voxel_sphere"):
            c = BillinearInterpolationVoxelSphere(self.meridian, self.latitude, p, c, max_dim)
        elif (self.interpolation == "multi_voxel_sphere"):
            c = BillinearInterpolationMultiVoxelSphere(self.meridian, self.latitude, self.rad_dim, p, c, max_dim)
